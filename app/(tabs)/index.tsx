...
export default function TabTwoScreen() {
  ...
  const weekName = { weekday: "long" as const }; //option
  const monthName = { month: "long" as const }; //option
  ...
  const nextDeadline = deadlines.reduce((prev, curr) =>
    curr < prev && curr > currentDate ? curr : prev
  );
  //nchufu a9reb deadline positif comparaison entre current deadline et previous deadline && current deadline et currentDate
    ...
    <ParallaxScrollView>
      ...
      <ThemedView style={styles.countdown}>
        <ThemedText style={styles.numberss}>
          {diffDays} <Text style={styles.span}>Days</Text>
        </ThemedText>
        <ThemedText style={styles.numberss}>
          {diffHours} <Text style={styles.span}>Hrs</Text>
        </ThemedText>
        <ThemedText style={styles.numberss}>
          {diffMinutes} <Text style={styles.span}>Min</Text>
        </ThemedText>
        <ThemedText style={styles.numberss}>
          {diffSeconds} <Text style={styles.span}>Sec</Text>
        </ThemedText>
      </ThemedView>
    </ParallaxScrollView>
  );
